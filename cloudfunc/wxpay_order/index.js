...
exports.main = async (event, context) => {
  ...
  app.router('pay', async (ctx) => {
    ...
    // 在云函数参数中，提取商品 ID
    let orderid = event.code;
    // 根据商品的数据库_id将其它数据提取出来
    let order = await db.collection('moon_tb_orders').doc(orderid).get();
    //ctx.body = order.data;
    //return;
    ...
    let result = await api.getPayParams({
      ...
      body: order.data.goodsList[0].name,       //商品名称，我设置的书名
      total_fee: order.data.totalPrice * 100,     //金额，注意是数字，不是字符串
